...
if len(sys.argv) >= 4:
	...
	for x in range(len(imgEntries)):
	
	
		
		cSpot = 0
		for y in range(len(csvEntries)-1):
			if imgEntries[x].time > csvEntries[y].time and imgEntries[x].time <= csvEntries[y+1]:
				cSpot = y
		
			
		newCombined = CombinedEntry(csvEntries[cSpot], imgEntries[x])
